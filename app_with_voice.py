...
✔️ 입력하신 글·음성은 **답변 생성에만 사용되며 저장되지 않습니다.** 새로고침하면 대화가 모두 지워져요.

🙅 주소·전화번호·주민번호 등 **민감한 개인정보는 입력하지 마세요.**

👶 어린이는 **성인 보호자와 함께** 이용해 주세요.

※ 서비스 개선을 위한 익명 통계(접속 시간, 클릭 등)는 수집될 수 있습니다.
"""
                ...
                with st.spinner(ui_text.get(language_mode, ui_text["한국어"])['spinner_llm']):
                    ...
                    if language_mode != "한국어":
                        _lang_override = {
                            ...
                            "日本語": "[出力言語指定: 日本語] 上の質問が韓국어であっても、必ず日本語だけで答えてください。場所名はシステムプロンプトのグロッサリーに従い、「日本語名称 (English Official Name)」の形式で記してください（例: 考えるゾーン (Thinking Zone)）。韓国語文字をそのまま出力しないこと。",
