...
def _extract_zone_keywords(zone_rows, top_n=12):
    ...
    stop = {
        "전시", "전시물", "체험", "놀이", "놀이터", "과학", "원리", "사용", "가능", "합니다", "것", "수", "통해", "있는",
        "위해", "때", "더", "그리고", "또한", "해서", "되는", "있어요", "하세요", "보기",
    }
...
def _get_ui_glossary_rules(language_mode: str) -> str:
    glossary = {
        "한국어": {
            "놀이터": "Zone(또는 놀이 공간)",
            "전시물": "exhibit(체험 요소)",
            "과학원리": "science principle(핵심 과학 개념)",
            "오디오북": "audiobook(음성 동화)",
        },
        "English": {
            "놀이터": "Zone",
            "전시물": "Exhibit",
    ...
    if language_mode == "한국어":
        return ""  # Korean prompts already use Korean.
...
def render_post_visit_learning(
    ...
    texts = {
        "한국어": {
            ...
            "expander_parent": "보호자용: 전시물 전체 보기",
