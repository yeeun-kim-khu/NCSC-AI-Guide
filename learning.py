...
def _render_quiz_card(zone_name: str, keyword: str, quiz_obj, language_mode: str = "한국어"):
    ...
    # 정답 토글: 문제 텍스트 해시를 키에 포함 → 새 문제 생성 시 자동으로 닫힌 상태로 시작
...
'{zone_name}'의 '{principle}' 주제로 4지선다 퀴즈를 만들어주세요.{detail_section}
...
- 따옴표/JSON 문법 정확히 지킬 것.""",
        ...
        "English": f"""Create a 4-choice quiz for children about '{principle}' from '{zone_name}'.{glossary_rules}{detail_section}
...
'{zone_name}'の'{principle}'をテーマに、子ども向け4択クイズを作ってください。{detail_section}
...
请围绕'{zone_name}'中的'{principle}'，为儿童设计一道四选一测验。{detail_section}
...
def generate_science_story(zone_name, exhibits, principles, language="한국어"):
    ...
    # 분위기 재료 (다음 5개 전시물 title) — 세계관에 자연스럽게 흙뿌릴 풍경/소품
    atmosphere_titles = []
...
▶ 분위기 재료(이야기 곳곳에 풍경/소품/등장 생물로 자연스럽게 흙뿌려 등장시킬 것 — 최소 2개 이상 본문에 포함):
   {atmosphere_summary}
...
[开展规则 — 至关重要]
1) **紧凑的三幕结构（共6〜8段）— 科学现象是故事的主干脈梁**:
   - 第一幕（2段）: 『{protagonist}』的平凡时刻 → **直接与『{principles_text}』相关的奇怪事件** → 一个明确目标（"我要弄清楚为什么…"）。
   - 第二幕（3〜4段）: 摆弄科学道具 → **现象小小地发生一次**（用五感描写）→ 失败一次 → 与伙伴一起观察同一现象反复出现 → "哦？怎么每次都这样！"——发现 **规律**。
